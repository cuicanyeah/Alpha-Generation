  ...
  switch (ComponentFunction()) {
    case kSetupComponentFunction: {
      if (!algorithm->setup_.empty()) {
        // std::cout << "1" << std::endl;
        InstructionIndexT index = InstructionIndex(algorithm->setup_.size(), kSetupComponentFunction);
    ...
    case kPredictComponentFunction: {
      if (!algorithm->predict_.empty()) {
        // std::cout << "2" << std::endl;
        InstructionIndexT index = InstructionIndex(algorithm->predict_.size(), kPredictComponentFunction);
    ...
    case kLearnComponentFunction: {
      if (!algorithm->learn_.empty()) {
        // std::cout << "3" << std::endl;
        // std::cout << "alter" << algorithm->learn_.size() << std::endl;
        InstructionIndexT index = InstructionIndex(algorithm->learn_.size(), kLearnComponentFunction);
...
void Mutator::RandomizeInstruction(Algorithm* algorithm) {
  switch (ComponentFunction()) {
    ...
    case kLearnComponentFunction: {
      if (!algorithm->learn_.empty()) {
        // std::cout << "6" << std::endl;
        // std::cout << "random" << algorithm->learn_.size() << std::endl;
        InstructionIndexT index = InstructionIndex(algorithm->learn_.size(), kLearnComponentFunction);
        algorithm->learn_[index] =
...
void Mutator::InsertInstruction(Algorithm* algorithm) {
  ...
  switch (ComponentFunction()) {
    case kSetupComponentFunction: {
      ...
      InsertInstructionUnconditionally(op, component_function, kSetupComponentFunction);
      break;
    }
    case kPredictComponentFunction: {
      ...
      InsertInstructionUnconditionally(op, component_function, kPredictComponentFunction);
      break;
    }
    case kLearnComponentFunction: {
      ...
      InsertInstructionUnconditionally(op, component_function, kLearnComponentFunction);
      break;
    }
  }
}
...
void Mutator::RemoveInstruction(Algorithm* algorithm) {
  ...
  switch (ComponentFunction()) {
    case kSetupComponentFunction: {
      ...
      component_function = &algorithm->setup_;
      RemoveInstructionUnconditionally(component_function, kSetupComponentFunction);
      break;
    }
    case kPredictComponentFunction: {
      if (algorithm->predict_.size() <= predict_size_min_) return;
      component_function = &algorithm->predict_;
      RemoveInstructionUnconditionally(component_function, kPredictComponentFunction);
      break;
    }
    case kLearnComponentFunction: {
      if (algorithm->learn_.size() <= learn_size_min_) return;
      component_function = &algorithm->learn_;
      RemoveInstructionUnconditionally(component_function, kLearnComponentFunction);
      break;
    }
  }
}
...
void Mutator::InsertInstructionUnconditionally(
    const Op op, vector<shared_ptr<const Instruction>>* component_function, const ComponentFunctionT component_functionT) {
  const InstructionIndexT position =
      InstructionIndex(component_function->size() + 1, component_functionT);
  if (component_functionT == kPredictComponentFunction && position < 11) return;
...
void Mutator::RemoveInstructionUnconditionally(
    vector<shared_ptr<const Instruction>>* component_function, const ComponentFunctionT component_functionT) {
  CHECK_GT(component_function->size(), 0);
  const InstructionIndexT position =
      InstructionIndex(component_function->size(), component_functionT);
  if (component_functionT == kPredictComponentFunction && position < 11) return;
...
InstructionIndexT Mutator::InstructionIndex(
    const InstructionIndexT component_function_size, const ComponentFunctionT component_functionT) {
  IntegerT op_index;
  switch (component_functionT) {
    case kSetupComponentFunction: {
      op_index = absl::Uniform<InstructionIndexT>(
      *bit_gen_, 5, component_function_size);
      // std::cout << "op_setup" << op_index << std::endl;
      break;
    }
    case kPredictComponentFunction: {
      while (true) {
        ...
        *bit_gen_, 0, component_function_size);
        // std::cout << "op_predict" << op_index << std::endl;
        if (op_index < 5 || (op_index > 5 && op_index < 11) || op_index > 15)
          break;
      }
      break;
    }
    case kLearnComponentFunction: {
      // std::cout << "learn_" << component_function_size << std::endl;
      op_index = absl::Uniform<InstructionIndexT>(
      *bit_gen_, 11, component_function_size);
      // std::cout << "op_learn" << op_index << std::endl;
      break;
    }
  }
  // std::cout << "final op: " << op_index << std::endl;
  return op_index;
}
