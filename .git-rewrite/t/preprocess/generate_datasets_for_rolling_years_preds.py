...
class EOD_Preprocessor:
    ...
    def make_your_own_trading_dates(self, begin_date):
        ticker_index = {}
        for index, ticker in enumerate(self.tickers):
        ...
        for ticker in self.tickers:
            ...
            for date_index, daily_EOD in enumerate(single_EOD):
                ...
                if cur_date > begin_date and cur_date < datetime.strptime('2012-11-19 00:00:00', self.date_format):        
                    cur_list = []
                    for col in range(1, len(daily_EOD)):
                        cur_list.append(float(daily_EOD[col]))
                        # print('cur_list',cur_list)
                    if np.isnan(np.min(cur_list)) or np.min(cur_list) < -1000: # -1234
                        if cur_date in isnan_dates:
                            isnan_dates[cur_date] += 1
                        else:
                            isnan_dates[cur_date] = 1
                    trading_dates.append(cur_date)
                    # print('len(trading_dates)',len(trading_dates))
        trading_dates = list(set(trading_dates))
        trading_dates.sort()
        no_nan_trading_dates = []
        for date in trading_dates:
            if date in isnan_dates:
                if isnan_dates[date] > 2:
                    continue
            no_nan_trading_dates.append(date.strftime("%Y-%m-%d %H:%M:%S"))

        print('sorted trading_dates',trading_dates)
        with open(os.path.join(self.data_path, '..',
                         self.market_name + '_added_dates.csv'), 'w', encoding='UTF8') as f:
            writer = csv.writer(f)
            writer.writerows(no_nan_trading_dates)
        sys.exit()
        return no_nan_trading_dates

    def generate_feature(self, selected_tickers_fname, begin_date, ith_data=0,
        ...
        # load from a list of dates to exclude holidays made only for 2012-11-19 onwards
        ...
        if begin_date < datetime.strptime('2012-11-19 00:00:00', self.date_format):
            if os.path.exists(os.path.join(self.data_path, '..',
                         self.market_name + '_added_dates.csv')):
                own_trading_dates = []
                with open(os.path.join(self.data_path, '..',
                                 self.market_name + '_added_dates.csv'), 'r', encoding='UTF8') as f:
                    for line in f:
                        own_trading_dates.append(''.join(re.split(',',line))[:-1])
                print('own_trading_dates', own_trading_dates)
                print('own_trading_dates[0]', own_trading_dates[0])
                print('trading_dates',trading_dates)
                print('trading_dates[0]',trading_dates[0])
                print('trading_dates[1]',trading_dates[1])
                all_trading_dates = []
                for i in own_trading_dates:
                    all_trading_dates.append(i)
                ...
                sys.exit()
                trading_dates = self.make_your_own_trading_dates(begin_date) + trading_dates 
            print('len(trading_dates)', len(trading_dates))
            print('-232*ith_data',-232*ith_data)
            trading_dates = trading_dates[:-232*ith_data] # 232 is because 244 - 12
            print('len(trading_dates)', len(trading_dates))
            trading_dates = trading_dates[-1275:]
            print('len(trading_dates)', len(trading_dates))

        # james: if more than or equal to 1275, [-1275:] will give 1275. 
        # james: if less than, will give error in the next assert
        # james: will get 1244 after the later mv processing
        assert (len(trading_dates) == 1275)
        print(trading_dates)
...
flags.DEFINE_integer('ith_data', 0,
                     'Generate one dataset for each previous year.')
...
def main(argv):
    ...
    ith_data = FLAGS.ith_data
    if ith_data > 0:
        FLAGS.data_dir = FLAGS.data_dir+str(ith_data)+'year_before_2012_11_29'

    begin_date = '2012-11-19 00:00:00'
    new_begin_date = str(2012 - ith_data)
    begin_date = begin_date[4:]
    begin_date = new_begin_date + begin_date

    stock_features, stock_labels = processor.generate_feature(
      processor.market_name + '_tickers_qualify_dr-0.98_min-5_smooth.csv',
      datetime.strptime(begin_date, processor.date_format),
      ith_data,
      return_days=1,
      pad_begin=29
    )
